...
const MENU_ITEMS = [
  { id: 101, category: "Kopi",     name: "Espresso Bold",        price: 25000, desc: "Ekstrak kopi murni dengan tekanan tinggi, pahit-pekat buat yang butuh suntikan energi cepat.", img: espressoImg,        featured: true },
  { id: 102, category: "Kopi",     name: "Caramel Macchiato",    price: 35000, desc: "Espresso, susu creamy, dan sirup karamel emas — manis pas tanpa bikin enek.", img: caramelImg,          featured: true },
  { id: 103, category: "Kopi",     name: "Palm Sugar Coffee",    price: 28000, desc: "Kopi susu khas Bogeng dengan manis legit gula aren asli, favorit pelanggan setia.", img: palmSugarImg,       featured: true },
  { id: 104, category: "Non-Kopi", name: "Matcha Latte Premium", price: 32000, desc: "Bubuk matcha kualitas perkebunan Uji dipadu susu segar, cocok buat yang gak minum kopi.", img: matchaImg,          featured: true },
  { id: 105, category: "Non-Kopi", name: "Choco Creamy Lava",    price: 30000, desc: "Cokelat lumer pekat dengan foam susu tebal di atasnya, manis dan memanjakan lidah.", img: chococreamylavaImg, featured: true },
  { id: 106, category: "Non-Kopi", name: "Red Velvet Cream",     price: 30000, desc: "Red velvet creamy dengan cream cheese lembut, manis seimbang gak berlebihan.", img: redvelvetImg,       featured: true },
  { id: 201, category: "Makanan",  name: "Nasi Goreng Spesial",  price: 25000, desc: "Nasi goreng rumahan dengan telur, ayam suwir, dan kerupuk renyah.", img: nasigorengImg },
  { id: 202, category: "Makanan",  name: "Sandwich Smoked Beef", price: 27000, desc: "Roti gandum lapis smoked beef, telur, dan saus mustard madu.", img: sandwichImg },
  { id: 203, category: "Makanan",  name: "Spageti Aglio Olio",   price: 28000, desc: "Spageti aglio olio dengan bawang putih sangrai dan taburan parmesan.", img: spagetiImg },
  { id: 204, category: "Cemilan",  name: "Dimsum Ayam",          price: 20000, desc: "Dimsum ayam kukus lembut, disajikan dengan saus sambal rumahan.", img: dimsumImg },
  { id: 205, category: "Cemilan",  name: "Cireng Bumbu Rujak",   price: 15000, desc: "Cireng renyah dengan bumbu rujak pedas-manis ala Riau.", img: cirengImg },
  { id: 206, category: "Cemilan",  name: "French Fries Crispy",  price: 18000, desc: "Kentang goreng renyah di luar, lembut di dalam.", img: frenchImg },
];

const MEMBER_TIERS = [
  { name: "Reguler",      icon: Coffee, req: "Aktif otomatis sejak transaksi pertama",                              benefit: "Mulai kumpulkan poin dari setiap pembelian yang kamu lakukan di kasir." },
  { name: "Loyal Member", icon: Heart,  req: "Minimal 10x transaksi dalam sebulan",                                 benefit: "Dapat diskon otomatis 5% setiap belanja, tanpa perlu kode promo apa pun." },
  { name: "VIP Member",   icon: Crown,  req: "Minimal 25x transaksi atau total belanja Rp500rb / bulan",            benefit: "Diskon otomatis 15% plus akses lebih dulu ke menu edisi terbatas." },
];

const FAQ_ITEMS = [
  { q: "Gimana cara naik level jadi Loyal atau VIP Member?",  a: "Sistem kasir kami otomatis menghitung jumlah transaksimu berdasarkan nomor HP yang terdaftar. Begitu syaratnya terpenuhi, level kamu naik sendiri tanpa perlu daftar ulang." },
  { q: "Diskon member bisa digabung sama promo lain, gak?",    a: "Diskon otomatis member berlaku sendiri di setiap transaksi supaya kamu selalu dapat harga terbaik secara transparan, tanpa drama hitung-hitungan." },
  { q: "Kalau mau pesan, harus lewat aplikasi?",              a: "Belum ada aplikasi pemesanan online untuk pelanggan. Untuk sekarang seluruh pesanan dilayani langsung oleh kasir di kedai." },
  { q: "Jam operasional Bogeng Coffee jam berapa?",           a: "Kami buka setiap hari pukul 08.00–22.00 WIB, termasuk akhir pekan dan hari libur nasional." },
];
  ...
  return (
    <div>
      <div className="flex gap-2 overflow-x-auto pb-2 mb-8 -mx-1 px-1">
        {categories.map((cat) => (
          <button key={cat} onClick={() => setActiveCat(cat)} data-cursor-hover
            className={`shrink-0 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wide transition-colors ${
              activeCat === cat ? "bg-[#2F2D2C] text-white" : "bg-white text-gray-500 border border-gray-200 hover:border-[#C67C4E]"
            }`}>
            {cat}
          </button>
        ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        {filtered.map((item) => {
          ...
          return (
            <motion.button key={item.id} onClick={() => onSelect(item)}
              animate={isSelected ? { scale: 1.05, y: -6 } : { scale: 1, y: 0 }}
              transition={{ duration: 0 }}
              whileHover={!isSelected ? { y: -4, scale: 1.03, transition: { duration: 0 } } : {}}
...
function ReviewForm() {
  ...
  return (
    <form onSubmit={handleSubmit}
      ...
      <p className="text-xs text-gray-400 mb-5">Ulasanmu akan ditinjau dulu oleh tim kami sebelum tampil di halaman ini.</p>
      <div className="flex gap-1 mb-4">
        {[1, 2, 3, 4, 5].map((n) => (
          <button type="button" key={n} data-cursor-hover
            onMouseEnter={() => setHoverStar(n)}
            onMouseLeave={() => setHoverStar(0)}
            onClick={() => setRating(n)}>
            <Star size={22} className="text-amber-400 transition-transform hover:scale-110"
              fill={(hoverStar || rating) >= n ? "currentColor" : "none"} strokeWidth={1.5} />
          </button>
        ))}
      </div>
      ...
      <textarea value={text} onChange={(e) => setText(e.target.value)}
        required rows={3} placeholder="Gimana rasanya ngopi di Bogeng?"
  ...
  return (
    <form onSubmit={handleSend} className="bg-[#FAF7F2] rounded-[28px] p-6 sm:p-8 border border-[#EFE6DC]">
      ...
      <textarea value={message} onChange={(e) => setMessage(e.target.value)}
        required rows={3} placeholder="Tulis keluhan atau saranmu di sini..."
      ...
      <div className="marquee-track">
        {doubled.map((rev, i) => (
          <div key={`${rev.id}-${i}`}
            ...
            <p className="text-xs text-gray-500 leading-relaxed italic my-3 line-clamp-4">"{rev.text}"</p>
            <div className="flex items-center gap-2.5 pt-3 border-t border-gray-200/60">
              ...
              <div>
                ...
                <span className="text-[9px] font-bold text-amber-700 uppercase tracking-wider">{rev.tier || "Pelanggan"}</span>
              </div>
            </div>
          </div>
        ))}
    ...
    <AnimatePresence>
      {visible && (
        <motion.button
          ...
          data-cursor-hover aria-label="Kembali ke atas"
...
// NAVBAR
// =====================================================================
function Navbar({ onGoTo }) {
  // FIX: state name konsisten → mobileOpen (bukan menuOpen)
  ...
  // FIX: Semua navigasi keluar pakai onGoTo (dengan overlay)
  const handlePortalMember = () => {
    setMobileOpen(false);
    onGoTo("/member-login");
  };
  const handleMasuk = () => {
    ...
    <nav className="fixed top-0 left-0 right-0 z-[100] px-6 sm:px-10 py-4 bg-white/85 backdrop-blur-md border-b border-gray-100">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {/* FIX: Portal Member pakai onGoTo bukan navigate langsung */}
          <motion.button onClick={handlePortalMember} data-cursor-hover
          ...
          <motion.button onClick={handleMasuk} data-cursor-hover
            whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.94 }}
            transition={{ type: "spring", stiffness: 400, damping: 20 }}
          ...
          {/* FIX: mobileOpen (bukan menuOpen) */}
          <button onClick={() => setMobileOpen((v) => !v)} data-cursor-hover
            className="md:hidden text-[#2F2D2C]" aria-label="Buka menu navigasi">
        ...
        {mobileOpen && (
          <motion.div
            ...
            <div className="flex flex-col gap-1 pt-4 pb-2">
              ...
              <motion.button onClick={handlePortalMember} whileTap={{ scale: 0.96 }}
              ...
              <motion.button onClick={handleMasuk} whileTap={{ scale: 0.96 }}
...
export default function BogengLandingPage() {
  ...
  // FIX: Satu hook untuk semua navigasi keluar
  const { leaving, goTo } = usePageTransition();
    ...
    <div className="relative min-h-screen bg-white text-[#2F2D2C] overflow-x-hidden selection:bg-[#C67C4E] selection:text-white font-sans">
      ...
      <RevealSection id="menu"
        ...
        <div className="max-w-6xl mx-auto px-6 sm:px-10 relative z-10">
          <div className="text-center mb-10">
            <span className="text-[10px] font-black text-[#C67C4E] uppercase tracking-[0.3em] block mb-2">Koleksi Rasa</span>
            <h2 className="text-2xl sm:text-3xl font-black font-serif italic text-[#2F2D2C]">Menu Lengkap Bogeng</h2>
          </div>
          <div className="mb-10">
      ...
      <RevealSection id="member" className="relative py-20 sm:py-24 bg-[#FAF7F2] z-10">
        <div className="max-w-6xl mx-auto px-6 sm:px-10">
          <div className="max-w-lg mb-14">
            <span className="text-[#C67C4E] font-black uppercase tracking-[0.25em] text-[10px] mb-2 block">Keuntungan Pelanggan Setia</span>
            <h2 className="text-2xl sm:text-3xl font-black font-serif italic text-[#2F2D2C] mb-3">Makin Sering Mampir, Makin Banyak Untungnya</h2>
            <p className="text-sm text-gray-500 leading-relaxed">
              Gak perlu daftar ribet. Cukup kasih nama atau nomor HP saat bayar, sistem kasir kami yang ngitung semuanya buat naikin levelmu otomatis.
            </p>
          </div>
            ...
            {MEMBER_TIERS.map((tier, idx) => {
              ...
              return (
                <motion.div key={tier.name}
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="bg-white p-6 rounded-2xl border border-gray-100 cursor-default">
                  <motion.div
          ...
          <motion.div
            ...
            <div>
              <p className="text-[10px] font-black text-[#C67C4E] uppercase tracking-[0.3em] mb-2">Sudah terdaftar?</p>
            ...
            {/* FIX: pakai goTo bukan navigate */}
            <motion.button onClick={() => goTo("/member-login")}
              whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.96 }} data-cursor-hover
      ...
      <RevealSection id="ulasan" className="relative py-20 sm:py-24 bg-white border-t border-gray-100 overflow-hidden z-10">
        <div className="text-center mb-12 px-6">
          <span className="text-[10px] font-black text-[#C67C4E] tracking-[0.3em] uppercase block mb-2">Suara Pelanggan</span>
          <h2 className="text-2xl sm:text-3xl font-black font-serif italic text-[#2F2D2C]">Kisah dari Cangkir Mereka</h2>
        </div>
        {(() => {
          const DUMMY = [
        ...
        <div className="px-6"><ReviewForm /></div>
      </RevealSection>
      ...
      <RevealSection id="faq" className="relative py-20 sm:py-24 bg-[#FAF7F2] z-10">
        <div className="max-w-2xl mx-auto px-6">
          <div className="text-center mb-10">
            <span className="text-[10px] font-black text-[#C67C4E] uppercase tracking-[0.3em] block mb-2">Sering Ditanya</span>
            <h2 className="text-2xl sm:text-3xl font-black font-serif italic text-[#2F2D2C]">Pertanyaan Umum</h2>
          </div>
          <div className="space-y-3">
            {FAQ_ITEMS.map((faq, idx) => (
              <motion.div key={idx}
                ...
                <button onClick={() => setActiveFaq(activeFaq === idx ? null : idx)} data-cursor-hover
      ...
      <RevealSection id="kontak" className="relative py-20 sm:py-24 bg-white z-10">
        <div className="max-w-5xl mx-auto px-6 sm:px-10 grid grid-cols-1 md:grid-cols-2 gap-8">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }}
            ...
            <span className="text-[10px] font-black text-[#C67C4E] uppercase tracking-[0.3em] block mb-2">Hubungi Kami</span>
            <h2 className="text-2xl sm:text-3xl font-black font-serif italic text-[#2F2D2C] mb-4">Kami Siap Dengar Ceritamu</h2>
      ...
      <style>{`
        @media (prefers-reduced-motion: reduce) {
          .leaf-anim, .bean-anim, .marquee-track { animation: none !important; opacity: 0.04 !important; }
        }
      `}</style>
    </div>
  );
