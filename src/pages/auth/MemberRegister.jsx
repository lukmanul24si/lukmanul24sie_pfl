...
const TIER_OPTIONS = [
  { value: 'Member', label: 'Member (Default)', desc: 'Mulai kumpulkan poin dari transaksi pertama' },
  { value: 'Loyal',  label: 'Loyal Member',     desc: 'Min. 10× transaksi — diskon 5% otomatis' },
  { value: 'Vip',    label: 'VIP Member',        desc: 'Min. 25× transaksi — diskon 15% + akses eksklusif' },
];
...
export default function MemberRegister() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!form.fullName.trim() || !form.username.trim() || !form.phoneNumber.trim()) {
    ...
    try {
      ...
      localStorage.setItem('bogeng_member_session', JSON.stringify({
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 text-white">
            ...
            <span className="text-xs font-black uppercase tracking-wider">Daftar Gratis</span>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {success ? (
              ...
              >
                ...
                <h2 className="text-2xl font-black text-[#2F2D2C] mb-2">Pendaftaran Berhasil!</h2>
                <p className="text-sm text-gray-400 font-bold mb-1">
                  Selamat datang, <span className="text-[#C67C4E]">{form.fullName}</span>!
                </p>
                <p className="text-xs text-gray-400 mb-6">
                  Kamu sudah terdaftar sebagai member Bogeng. Kamu akan diarahkan ke portal member…
                </p>
                {/* Progress bar */}
              ...
              <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <div className="mb-7">
                  ...
                  <p className="text-sm text-gray-400 leading-relaxed">
                    Daftar gratis dan nikmati keuntungan eksklusif member Bogeng Coffee.
                  </p>
                </div>
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-wider block ml-1">
                      Username <span className="text-gray-300 normal-case font-bold">(dipakai untuk login)</span>
                    </label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-wider block ml-1">
                      Nomor HP <span className="text-gray-300 normal-case font-bold">(dipakai untuk login)</span>
                    </label>
                    <div className="relative">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={set('membershipLevel')}
                        className="w-full px-5 py-3.5 bg-white border-2 border-[#EFE6DC] hover:border-[#C67C4E]/40 focus:border-[#C67C4E] rounded-2xl outline-none font-bold text-sm text-[#2F2D2C] transition-all duration-200 shadow-sm appearance-none cursor-pointer pr-10"
                      >
                        {TIER_OPTIONS.map(o => (
                    ...
                    {/* Deskripsi tier yang dipilih */}
                    <p className="text-[10px] text-gray-400 font-bold ml-1">
                      {TIER_OPTIONS.find(o => o.value === form.membershipLevel)?.desc}
                    </p>
                  </div>
                ...
                <div className="mt-6 grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={i} className="bg-white border border-[#EFE6DC] rounded-xl p-2.5 text-center">
                      <span className="text-lg block mb-1">{item.emoji}</span>
                      <p className="text-[9px] font-black text-gray-400 leading-tight">{item.text}</p>
                    </div>
                  ))}
                </div>
